...
public class GUINhanVien extends GUIFormContent {
                ...
                if (a == JOptionPane.YES_OPTION) {
                    if (checkTextThem(txt_NhanVien_Them[1].getText(),
                        ...
                        TaiKhoanDTO tk = new TaiKhoanDTO(tenTaiKhoan.trim(),
                                ...
                                pqBUS.traVeMaQuyenTuTenChucVu(cbChucVu_Them.getSelectedItem().toString()),
                                "123456",
                                "Hiện");
    ...
    private void Sua_Frame(int row) {
        JFrame f = new JFrame();
        //Để cờ hiệu với giá trị 0 với ý nghĩa không được bấm ra khỏi Dialog trừ nút Thoát
        cohieu = 0;
        Sua = new JDialog(f);
        ...
        // Ẩn cbChucVu nếu đang là tài khoản đăng nhập hoặc chưa ai có chức vụ đó
        ...
        TaiKhoanDTO tkDTO = tkBUS.getTaiKhoanTheoMaNhanVien(maNhanVien);
        if(maNhanVien.equals(Tool.IDNhanVienHienHanh) || tkBUS.demNhanVienTheoMaQuyen(tkDTO.getIDPhanQuyen()) <= 1){
            cbChucVu_Sua.setEnabled(false);
        }
    ...
    protected void Them_click(MouseEvent evt) {
        array_ChucVu=PhanQuyenBUS.getChucVu();
        Them_Frame();
    }
    ...
    protected void Xoa_click(MouseEvent evt) {
        ...
        if (row == -1) {
            ...
            if (option == JOptionPane.YES_OPTION) {
                ...
                TaiKhoanDTO tkDTO = tkBUS.getTaiKhoanTheoMaNhanVien(maNhanVien);
                if(maNhanVien.equals(Tool.IDNhanVienHienHanh)){
                    JOptionPane.showMessageDialog(null, "Không thể xóa nhân viên đang đăng nhập");
                    return;
                }
                else if(tkBUS.demNhanVienTheoMaQuyen(tkDTO.getIDPhanQuyen()) <= 1){
                    JOptionPane.showMessageDialog(null, "Không thể xóa nhân viên khi chưa có ai đảm nhận chức vụ này");
                    return;
                }
                else {
                    ...
                    tkBUS.xoa(tkBUS.getTaiKhoanTheoMaNhanVien(maNhanVien), tkBUS.timViTriMaNhanVien(maNhanVien));
                }
            }
        }
