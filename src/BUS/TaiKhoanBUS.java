...
public class TaiKhoanBUS {
   ...
   public TaiKhoanBUS()
    ...
    //tìm vị trí của thằng có chứa mã mà mình cần
    public static int timViTriMaNhanVien( String IDNhanVien) 
    {
        for (int i = 0; i < dstk.size(); i++) {
            if (dstk.get(i).getIDNhanVien().equals(IDNhanVien)) {
                return i;
            }
        }
        return 0;
    ...
    public TaiKhoanDTO getTaiKhoanTheoMaNhanVien(String maNhanVien)
    {
        for(TaiKhoanDTO tkDTO : dstk)
        {
            if(tkDTO.getIDNhanVien().equals(maNhanVien))
            {
                return tkDTO;
            }
        }
    ...
    public static int demNhanVienTheoMaQuyen(String maQuyen)
    {
        int count = 0;
        for(TaiKhoanDTO tkDTO : dstk)
        {
            if(tkDTO.getIDPhanQuyen().equals(maQuyen))
            {
                count ++;
            }
        }
